...
class UserIdInput extends React.Component {
  ...
  render() {
    ...
    const { isLoading, userId: loadedUserId } = this.props;
    const suffix = userId ? (
      <Icon type="close-circle" onClick={this.onClearUserId} />
    ) : null;

    console.log("userId == loadedUserId", userId, loadedUserId);
    return (
      <Form layout="inline" onSubmit={this.handleSubmit}>
        ...
        <Form.Item>
          <Button
            type="primary"
            htmlType="submit"
            disabled={isLoading || !userId || userId === loadedUserId}
            icon={isLoading ? "loading" : "download"}
          >
            {isLoading ? "Loading..." : "Load ascents!"}
          </Button>
        </Form.Item>
        <br />
        <br />
